function [ resultat ] = SeuillageDynamiqueParMoyenne( img )
...
image_resultat=zeros(nl,nc); %Creation de la nouvelle matrice qui sera la destination de notre image résultat
    ...
    pixelCourrant=img(i,j);                
      ...
      somme =sum(tab(:));           
			moyenne=somme/cpt;                   %Calcul de la moyenne de la fenetre 3*3.
		
     if (pixelCourrant<moyenne)
			image_resultat(i,j)=0;       % si intensite de pixel > moyenne il reçoi 1 
			else
